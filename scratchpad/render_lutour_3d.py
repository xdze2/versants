"""Build a self-contained isometric 3D demo of the Gave de Lutour catchment.

Takes the COP30 tile and the DEM-delineated catchment polygon from
``scratchpad/dem_lutour.py`` and bakes them into a single ``lutour_3d.html``:

* the elevation grid, clipped to the catchment outline, as a 16-bit heightmap
  PNG embedded as a data URI;
* the Lutour troncon network, with a height sampled from the DEM at every
  vertex, as an array of 3D polylines;
* a three.js scene (orthographic camera at the true isometric angle,
  hypsometric tint + hillshade, ~2x vertical exaggeration) that renders it all
  client-side. No server, no build step - open the file in a browser.

Run::

    uv run python scratchpad/render_lutour_3d.py

Everything it needs is already installed (rasterio, geopandas, shapely, numpy,
pillow); three.js loads from cdnjs at view time.
"""
...
CATCHMENT_GJ = OUT_DIR / "lutour_catchment_dem.geojson"
TRONCON = RAW_DIR / "troncon_hydrographique_pyrenees.parquet"
REF_RIVER_ID = "COURDEAU0000002000907698"
OUT_HTML = OUT_DIR / "lutour_3d.html"

# Vertical exaggeration applied in the scene. 1.0 = true scale.
Z_EXAGGERATION = 1.0

# Pad the clipped grid this many cells outside the catchment so the block has a
# thin apron of terrain around the divide instead of a knife edge.
PAD_CELLS = 6
...
def _clip_dem() -> tuple[np.ndarray, dict]:
    """Return (elevation array with NaN outside the catchment, meta dict)."""
    catch = gpd.read_file(CATCHMENT_GJ).to_crs(4326)
    geom = catch.geometry.iloc[0]
...
def _streams(meta: dict) -> list[list[list[float]]]:
    """Lutour troncons as polylines of [u, v], u/v normalised to the clipped grid.
    ...
    Vertices are densified to <=~40 m spacing so a long segment still follows the
    surface between its original nodes.
    """
    tr = gpd.read_parquet(TRONCON).to_crs(4326)
    ...
    for geom in lines.geometry:
        ...
        for part in parts:
            ...
            for (lon0, lat0), (lon1, lat1) in zip(raw, raw[1:]):
...
HTML = """<title>Gave de Lutour 3D</title>
<style>
...
<canvas id="cv"></canvas>
<div id="hud">
  <h1>Gave de Lutour &mdash; catchment from COP30</h1>
  <p id="stats"></p>
</div>
<div id="tip">drag to orbit &middot; scroll to zoom &middot; {exag}&times; vertical &middot; contours 20 m</div>
...
def main() -> None:
    ...
    streams = _streams(meta)
    minor, index = _contours(band, meta)
    print(f"[dem] clipped {meta['rows']}x{meta['cols']}, "
          f"{meta['n_valid']} valid cells, z {meta['z_min']:.0f}-{meta['z_max']:.0f} m")
    print(f"[streams] {len(streams)} polylines")
    print(f"[contours] {CONTOUR_INTERVAL_M} m: {len(minor)} minor + {len(index)} "
          f"index polylines")
    ...
    print(f"[out] {OUT_HTML} ({len(html) / 1e6:.2f} MB)")
